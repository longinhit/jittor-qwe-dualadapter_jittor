...
'''LJW修改'''
class DatasetWrapper(jtDataset):
    def __init__(self, data_source, input_size, total_len, transform=None, is_train=False,
        ...
        to_tensor = []
        to_tensor += [jtf.Resize(input_size, mode=interp_mode)]
        to_tensor += [jtf.ToTensor()]
        normalize = jtf.ImageNormalize(
            mean=(0.48145466, 0.4578275, 0.40821073), std=(0.26862954, 0.26130258, 0.27577711)
        )
        to_tensor += [normalize]
        self.to_tensor = jtf.Compose(to_tensor)

    def __getitem__(self, k):
        # 返回数据的逻辑
        item = self.data_source[k] # <class 'datasets.utils.Datum'>
        output = {
            'label': item.label, # class label.
            'domain': item.domain, # domain label.
            'impath': item.impath # image path.
        }

        img0 = read_image(item.impath) # <class 'PIL.Image.Image'>

        if self.transform is not None: 
            if isinstance(self.transform, (list, tuple)): # train_tranform
                ...
                img = self._transform_image(self.transform, img0) # <class 'jittor.jittor_core.Var'>
                output['img'] = img
        
        if self.return_img0: # false
            output['img0'] = self.to_tensor(img0) 

        return output['img'], output['label'] # <class 'dict'> 
    
    def _transform_image(self, tfm, img0):
        img_list = []
        for k in range(self.k_tfm):
            image = tfm(img0)
            img_list.append(tfm(img0)) # <class 'jittor.jittor_core.Var'>

        img = img_list # <class 'list'>
        if len(img) == 1:
            img = img[0]
...
def build_data_loader(
    ...
    '''LJW修改'''
    dataset = dataset_wrapper(data_source, input_size=input_size, total_len=len(data_source), transform=tfm, is_train=is_train).set_attrs(
        batch_size=batch_size,
        shuffle=shuffle,
        num_workers=8,  # 根据需要调整
        drop_last=False  # 根据需要调整
    )
